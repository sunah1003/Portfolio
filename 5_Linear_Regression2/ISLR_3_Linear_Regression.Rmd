...
This markdown file is created by Sunah Park for extended lab exercises in the book 'An Introduction to Statistical Learning with Applications in R' [(ISLR)](https://www-bcf.usc.edu/~gareth/ISL/ISLR%20First%20Printing.pdf). \
...
The fitted model for single linear regression:
...
```
Diagnose: There is no increasing or decreasing pattern of residuals. The variable is also not in time-series structure.


## __2. Multiple Linear Regression__
```{r mlr-mod, echo=TRUE, include=TRUE} 
# Model selection
lm.fit<-lm(medv~., data=Boston) # y: medv, x: all predictors in Boston 
